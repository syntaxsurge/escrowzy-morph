...
[etherscan]\n`
  ...
  for (const [chainName, chainConfig] of Object.entries(yamlConfig.chains)) {
    ...
    if (!networkName.toLowerCase().includes('hedera')) {
      config += `${networkName} = { key = "", url = "${chainConfig.explorerUrl}", chain = ${chainConfig.chainId} }\n`
    }
  }
  config += `# Hedera uses Sourcify for verification, not Etherscan
# Morph L2 uses a custom block explorer\n`
...
deploy_to_network() {
    ...
    # Deploy with chain verification disabled to support all chains including custom ones
    # Using --skip-simulation and --legacy flags for better compatibility
    if [[ "$network" == "hederaTestnet" || "$network" == "hederaMainnet" ]]; then
        ...
        forge script script/Deploy.s.sol:DeployScript --rpc-url $network --private-key $DEPLOY_PRIVATE_KEY --broadcast --slow --legacy --skip-simulation
        ...
        # Universal deployment for all chains
        # Using --skip-simulation to bypass chain validation and --legacy for compatibility
        echo "Deploying to $network (using universal flags for compatibility)..."
        if [ "$verify_flag" = "--verify" ]; then
            forge script script/Deploy.s.sol:DeployScript --rpc-url $network --private-key $DEPLOY_PRIVATE_KEY --broadcast --legacy --skip-simulation --verify
        else
            forge script script/Deploy.s.sol:DeployScript --rpc-url $network --private-key $DEPLOY_PRIVATE_KEY --broadcast --legacy --skip-simulation
        fi
    fi
}
